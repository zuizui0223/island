...
# Complementary engine contract:
# - brms owns binary syndrome-level pathway inference (plain/non-plain,
#   generalized/non-generalized, SC/SI).
# - INLA owns the retained category-level decomposition.
# - The two engines are complementary and are not treated as duplicate votes.
n_input <- nrow(d)
...
# Northern-midlatitude binary pathway analysis.
...
meta <- list(
  contract = "v2_complementary_bayesian_binary_pathway_v1",
  role = "binary syndrome-level pathway inference complementary to category-preserving INLA",
  method = "Global isolation-by-regime falsification plus northern binary Bombus/SC pathway inference",
  analysis_tier = "sensitivity_all",
  binary_outcomes = c("SC_vs_SI", "plain_vs_non_plain", "generalized_vs_non_generalized"),
  category_preserving_models = false,
  inla_complement = "INLA estimates retained flower-colour and floral-form categories separately",
  ...
  northern_pathway = "Bombus deficit -> SC and direct/SC-mediated paths to binary plain/generalized outcomes",
  interpretation_guardrail = "Bayesian binary results describe syndrome-level paths; INLA category results identify which retained categories drive them. Agreement is not counted as duplicate independent evidence.",
