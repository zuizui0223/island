...
endpoints <- list(
  color=list(all_prefix='all_fine_color_', all_trials='all_fine_color_trials', animal_prefix='fine_color_', animal_trials='fine_color_trials', categories=colors),
  access=list(all_prefix='all_access_', all_trials='all_access_trials', animal_prefix='animal_access_', animal_trials='animal_access_trials', categories=access)
)
...
fit_comp <- function(data,endpoint,model_name,prefix,trials,categories,rhs){
  ...
  fx<-as.data.table(fit$summary.fixed,keep.rownames='parameter'); fx[,`:=`(endpoint=endpoint,model=model_name)]; fixed_rows[[length(fixed_rows)+1L]]<<-fx
  cpo<-fit$cpo$cpo; cpo<-cpo[is.finite(cpo)&cpo>0]; score_rows[[length(score_rows)+1L]]<<-data.table(endpoint=endpoint,model=model_name,n_islands=uniqueN(data$island_id),n_rows=nrow(long),log_cpo_sum=if(length(cpo))sum(log(cpo)) else NA_real_,waic=fit$waic$waic,dic=fit$dic$dic)
}
...
for(ep in names(endpoints)){
  ...
  fit_comp(global_support,paste0('all_flora_',ep),'G_global_isolation_by_regime_no_bombus',x$all_prefix,x$all_trials,x$categories,global_rhs)
  support_rows[[length(support_rows)+1L]]<-data.table(endpoint=paste0('all_flora_',ep),layer='global_primary_all_flora_no_bombus',n_islands=nrow(global_support))

  mech_vars<-c('z_log_distance_to_continent_km','z_isolation_sq','z_log_island_area_km2','z_climate_pc1','z_climate_pc2','z_climate_pc3','z_climate_pc4','z_bombus_deficit','z_sc_share',paste0('z_',poll_controls))
  ...
  m<-list(N0_isolation=base_rhs,N1_bombus=paste(base_rhs,'+ category:z_bombus_deficit'),N2_bombus_sc=paste(base_rhs,'+ category:z_bombus_deficit + category:z_sc_share'))
  extra<-c('category:z_bombus_deficit','category:z_sc_share',paste0('category:z_',poll_controls)); m$N3_full<-paste(base_rhs,'+',paste(extra,collapse=' + '))
  for(nm in names(m)) fit_comp(ns,paste0('animal_flora_',ep),nm,x$animal_prefix,x$animal_trials,x$categories,m[[nm]])
  support_rows[[length(support_rows)+1L]]<-data.table(endpoint=paste0('animal_flora_',ep),layer='northern_bombus_mechanism',n_islands=nrow(ns))
}
...
sc_fx<-as.data.table(sc_fit$summary.fixed,keep.rownames='parameter'); sc_fx[,`:=`(endpoint='self_compatibility',model='SC_bombus_reproductive_assurance_north_only')]; fixed_rows[[length(fixed_rows)+1L]]<-sc_fx
...
# Explicit global-regime isolation table; tropical and southern rows come only from the no-Bombus global model.
...
meta<-list(primary_endpoints=c('fine colour composition','functional floral access-structure composition'),access_classes=access,global_model='all trait-resolved flora; Bombus excluded from all regimes',tropical_southern_interpretation='isolation effects estimated without Bombus terms because Bombus is not the regional baseline channel',northern_mechanism='animal-pollinated flora only: isolation + Bombus deficit + SC + estimable pollination-mode controls',joint_colour_form='sensitivity only')
