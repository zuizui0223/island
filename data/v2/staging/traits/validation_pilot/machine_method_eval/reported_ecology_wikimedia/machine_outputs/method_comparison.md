...
- Use `machine_pollinator_guild_index` as a derived no-review visitor guild layer: 0 species rows.
...
- `machine_trait_selected.csv`: 2 selected species-trait rows
- `machine_trait_sensitivity.csv`: 1 sensitivity/proxy rows
...
| llm_reported_ecology_excerpt | 3 | 3 | 0.06 | 3 | 0 | next_main_text_lane_for_reproduction_and_visitors | Source-excerpted reproductive, selfing, pollen-vector, or visitor statements; usable as main no-review text evidence when species-direct. |
...
| machine_pollinator_guild_index | 0 | 0 | 0.0 | 0 | 0 | derived_no_review_pollinator_guild_layer | Derived from source-backed GloBI partner taxa; use as a machine guild index, not human-verified ecology. |
