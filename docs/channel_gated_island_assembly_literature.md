...
### 「鳥・蝶は移動でき、派手な花を好む」の精密化

この直感はDiscussion仮説として使えるが、`鳥・蝶`を一群にして一般化しては
ならない。一次研究から支持される分解は次である。

1. **遠隔島への到達:** 特殊化した *Epicephala* 蛾と宿主植物の相利系は、
   太平洋の遠隔島へ反復移住している
   （[Hembry et al. 2011](https://doi.org/10.1098/rsbl.2011.0771)）。これは
   Lepidopteraの一部が特殊化チャネルを島で再成立させ得る証拠であり、全蝶類が
   Bombusより高い分散力をもつという証拠ではない。
2. **蝶の視覚シグナル:** 蝶は花色相を識別・学習して採餌を変える
   （[Weiss 1991](https://doi.org/10.1038/354227a0)）。これは色シグナル維持の
   機構を支えるが、島間移住や植物の長期進化を直接測定していない。
3. **鳥による再加入・部分代替:** New Zealandではrecent-colonist birdsが、
   固有送粉者喪失後の送粉を部分的に補償した
   （[Pattemore & Wilcove 2012](https://doi.org/10.1098/rspb.2011.2036)）。
   `部分的`であって、全植物の完全代替ではない。
4. **鳥の色選択:** 豪州234種の反射スペクトルと鳥色空間の系統比較では、
   鳥媒植物80種の約半数が長波長反射の狭い領域へ収斂した
   （[Burd et al. 2014](https://doi.org/10.1098/rspb.2013.2862)）。鳥媒植物全体が
   一律に赤・派手になるわけではない。

したがってTropical/Southernの予測は `派手化の逆syndrome` ではなく、
Northern型単純化ベクトルの**減弱・欠如・異質化**とする。蝶は色、hawkmoth・
特殊蛾は長花筒・他殖、鳥は有効な残存・再加入と鳥視覚上の色信号という別々の
補助仮説をつくる。Chapter 1ではtrait vectorとの整合性まで、Chapter 2では
到達・訪花・per-visit effectiveness・結実による機能代替までを検証する。

重要なのは、後者がChapter 1の欠測変数ではないことである。Chapter 1は、既に
収集した花サイズ、筒深、相称性、花形、花色、繁殖適合性、自殖能力、mating
systemから地域別の残差ベクトルを確立し、文献で事前定義した送粉syndromeとの
整合性をDiscussionで論じるところまでで完結できる。
...
これらはChapter 1を完了させるための追加取得項目ではなく、Chapter 1のtrait
patternから導かれた因果候補を将来直接検証する場合の要件である。
