...
To align the discovery layer directly with H2, v14 additionally constructs two post-hoc family scores from the same frozen exact-species parent rows. The reproductive-assurance score is the weighted mean of self-compatibility, selfing mating system and autonomous selfing, with equal weights and at least two observed components; this is the binary-state equivalent of the H2 `selfing_core` construction. The accessibility/generalization score uses generalized form (weight 1.0), actinomorphy (0.75) and shallow/open tube (1.0), again requiring at least two observed components, matching the H2 `generalized_accessible` weighting. The same global model, publication-total weight and publication-cluster robust covariance are then applied with `family_score` replacing the atomic trait state. An equal-weight three-component accessibility score is retained as a sensitivity because it matches the prospectively frozen H4b definition in PR #236. These family-level analyses are post-hoc discovery refinements, not prospective validation.
...
The v14 family-aligned discovery strengthened the H2–H4 connection. A reproductive-assurance score constructed from at least two frozen self-compatibility, mating-system and autonomous-selfing components was available for 470 species across 417 publications and 739 measurement cells. Higher reproductive assurance was associated with lower current pollen limitation (`β=-0.31255`, SE `0.10218`, two-sided `p=0.00222`). The no-zero-constant sensitivity retained the association (`β=-0.31726`, `p=0.00229`), whereas the supplemental-only subset remained negative but was not supported (`β=-0.13138`, `p=0.148`).

The H2-exact weighted accessibility/generalization score was available for 228 species across 232 publications and 403 cells. Higher accessibility was associated with lower pollen limitation (`β=-0.32932`, SE `0.11771`, two-sided `p=0.00515`) and remained supported in both supplemental-only (`β=-0.24989`, `p=0.0367`) and no-zero-constant (`β=-0.30663`, `p=0.0134`) sensitivities. An equal-weight score matching the prospectively frozen H4b definition gave nearly the same result (`β=-0.32509`, `p=0.00636`). Thus both H2 plant-response families point toward lower current pollen limitation in the post-hoc discovery layer, with the accessibility bridge showing the more stable measurement sensitivity.
...
The family-level analysis closes the conceptual gap more directly. The H2-equivalent reproductive-assurance score and the H2-exact weighted accessibility/generalization score are both negatively associated with current pollen limitation in the global exact-species overlap. The accessibility family is especially coherent: its primary estimate is supported and remains negative and supported in both frozen measurement sensitivities. The equal-weight version later frozen prospectively for H4b produces nearly the same post-hoc estimate.
...
The evidence supports a four-step synthesis. First, island isolation is associated with a recurrent multivariate syndrome spanning reproductive assurance, a directional colour contrast and floral accessibility. Second, the floral component is not fully reducible to measured selfing: accessibility/generalization remains associated with isolation after conditioning on `selfing_core`, whereas colour contributes more heterogeneously. Third, independent pollen-supplementation experiments show that pollen limitation increases with isolation. Fourth, exact-species functional triangulation links both H2 response families to lower current pollen limitation: reproductive assurance is negative in the post-hoc family bridge, while the H2-exact accessibility/generalization score is negative in the primary model and both frozen measurement sensitivities.
