...
To align the discovery layer literally with H2, v14 first uses the **exact species-level Direct-only H2 scores** already produced by Chapter 1: `selfing_core` and `generalized_accessible` soft membership. Scientific names are matched exactly after only case-folding and underscore/space normalization, with no synonym or genus fallback. The same global GloPL model, publication-total weight and publication-cluster robust covariance are then applied with the exact H2 score replacing the atomic trait state.

A separate post-hoc sensitivity reconstructs comparable family scores directly from the frozen Route A/B atomic preflight states. Reproductive assurance uses self-compatibility, selfing mating system and autonomous selfing. Accessibility uses generalized form (weight 1.0), actinomorphy (0.75) and shallow/open tube (1.0), requiring at least two observed components. An equal-weight accessibility reconstruction is also retained because it matches the prospectively frozen H4b definition in PR #236. The reconstruction layer is supportive sensitivity only; it does not replace the literal H2-score bridge.
...
Second, the same two directions were frozen for an independent PolLimCrop agricultural-domain transportability test. Its outcome-blind support gate also failed before outcome unblinding. H4a matched 36 species across 109 publications but had only 9 state-0 species against a frozen minimum of 10. H4b matched 42 species across 141 publications, with score SD 0.353, but had only 6 low-score species against a frozen minimum of 8. No crop hypothesis was admitted, so row-level crop pollen-limitation outcomes remained unopened. This is support insufficiency, not a transportability null. Because the H1/H2 colour response is context dependent, v14 does not add a global primary colour-to-pollen-limitation bridge.
...
The original plant and GloPL global-distance models retain their frozen inferential roles. The v13 exact-species H4 remains explicitly **post-hoc discovery**. Both prospective validation routes remain separate and stopped before outcome unblinding because their frozen support gates were not met. The post-2015 wild cohort reached only 9 matched species for each co-primary family, while PolLimCrop failed the H4a state-0 species minimum (9 < 10) and the H4b low-score species minimum (6 < 8). Neither route therefore confirms nor biologically rejects H4, and no prospective outcome values were opened. No layer is interpreted as proving historical mediation from pollen limitation to trait evolution.
...
The v14 **exact-H2-score discovery** strengthened the H2–H4 connection without reconstructing the plant response. The literal Direct-only `selfing_core` score matched 455 GloPL species across 409 publications and 734 measurement cells. Higher `selfing_core` was associated with lower current pollen limitation (`β=-0.29706`, SE `0.10353`, two-sided `p=0.00411`). The no-zero-constant sensitivity retained the association (`β=-0.29990`, `p=0.00438`), whereas the supplemental-only subset remained negative but was not supported (`β=-0.09462`, `p=0.313`).

The literal Direct-only `generalized_accessible` score matched 143 GloPL species across 143 publications and 259 cells. Higher accessibility was associated with lower pollen limitation (`β=-0.29601`, SE `0.12940`, two-sided `p=0.0222`). The supplemental-only subset remained supported (`β=-0.30329`, `p=0.0384`); the no-zero-constant estimate remained negative (`β=-0.30399`, two-sided `p=0.0197`; one-sided negative `p=0.00986`).

The independent atomic-reconstruction sensitivity was concordant. Reconstructed reproductive assurance gave `β=-0.31255` (`p=0.00222`), while the H2-weighted reconstructed accessibility score gave `β=-0.32932` (`p=0.00515`) and remained supported in both measurement sensitivities. An equal-weight accessibility reconstruction matching the prospectively frozen H4b definition gave `β=-0.32509` (`p=0.00636`). Thus both **actual H2 species scores** point toward lower current pollen limitation in the post-hoc discovery layer, and the reconstructed-family sensitivities support the same interpretation.
...
The prospective validation layer does not upgrade that inference. The post-2015 wild-plant temporal replication stopped at its outcome-blind support gate: both co-primary families had only 9 matched species, below the frozen requirement of 30 species and 10 publications, so pollen-limitation outcomes were not opened. The independent PolLimCrop route also stopped at its outcome-blind support gate: H4a had 36 matched species but only 9 state-0 species (minimum 10), and H4b had 42 matched species but only 6 low-score species (minimum 8). Crop outcomes therefore also remained unopened. These are two support failures, not evidence against either biological prediction.
...
### Functional triangulation now connects both H2 response families

H4 asks a different question again: given the independent H3 pressure, are the H2 response families associated with lower current pollen limitation? At the atomic-trait level, autonomous selfing remains the strongest bridge, and its negative association persists through global sensitivities and within-study comparisons. Self-compatibility alone is weaker, consistent with the fact that compatibility permits self-fertilization but does not guarantee autonomous pollen transfer.

The family-level analysis closes the conceptual gap more directly because the primary v14 bridge now uses the **literal H2 species scores**. Both `selfing_core` and `generalized_accessible` are negatively associated with current pollen limitation in the exact-species overlap. `generalized_accessible` remains supported in both the supplemental-only and no-zero measurement sensitivities. Reconstructing comparable family scores from the H4 parent atomic states yields concordant results, including an equal-weight accessibility version later frozen prospectively for H4b.

This is still functional compatibility rather than historical mediation. The family scores were applied to the already inspected v13 outcome base and therefore remain post-hoc discovery. Their value is alignment: the same broad plant-response families that increase with island isolation in H2 are associated with lower current pollen limitation in H4. The prospective wild and crop-domain layers retain separate inferential roles.
...
The evidence supports a four-step synthesis. First, island isolation is associated with a recurrent multivariate syndrome spanning reproductive assurance, a directional colour contrast and floral accessibility. Second, the floral component is not fully reducible to measured selfing: accessibility/generalization remains associated with isolation after conditioning on `selfing_core`, whereas colour contributes more heterogeneously. Third, independent pollen-supplementation experiments show that pollen limitation increases with isolation. Fourth, exact-species functional triangulation links both **literal H2 species scores** to lower current pollen limitation: `selfing_core` and `generalized_accessible` are both negative and supported in the primary post-hoc bridge, with concordant atomic-reconstruction sensitivities.
...
Independent pollen-supplementation experiments show that pollen limitation also increases with geographic isolation. The functional bridge is strongest at the atomic level for autonomous selfing, and the v14 exact-score discovery additionally shows negative pollen-limitation associations for the actual H2 `selfing_core` and `generalized_accessible` species scores. A genuinely prospective wild-plant validation attempt remains non-evaluable because of frozen support limits, and an independent crop-domain test remains separate. Together, H1–H4 establish pattern, decomposition, ecological pressure and functional compatibility without claiming historical mediation. The next decisive step is lineage-resolved or longitudinal evidence linking pollination service, reproductive success and trait change through time.
