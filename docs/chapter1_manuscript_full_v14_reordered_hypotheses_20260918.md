...
A separate prospective audit was conducted after the H4 discovery and is **not part of the H4 result**. Both the post-2015 wild-plant route and the independent PolLimCrop route stopped at predeclared outcome-blind support gates before pollen-limitation outcomes were opened. The audit is retained only to document the confirmatory evidence boundary; it neither upgrades nor downgrades the post-hoc H4 functional bridge.
...
The original plant and GloPL global-distance models retain their frozen inferential roles. H4 remains explicitly **post-hoc functional triangulation** based on the exact-species discovery, literal H2-score bridge and atomic reconstruction sensitivity. The separate prospective audit is supplementary provenance only and is not counted as an H4 evidence layer. No analysis is interpreted as proving historical mediation from pollen limitation to trait evolution.
...
This is still functional compatibility rather than historical mediation. The family scores were applied to the already inspected v13 outcome base and therefore remain post-hoc discovery. Their value is alignment: the same broad plant-response families that increase with island isolation in H2 are associated with lower current pollen limitation in H4. The separate prospective audit is not used to classify the H4 result.
...
A separate prospective validation audit was attempted but is not promoted into H4. The post-2015 wild route and the PolLimCrop route both stopped before outcome unblinding because predeclared support gates were not met. These support-limited audits define what confirmatory evidence is still missing; they are not biological null results and do not change the post-hoc H4 conclusion.
