...
### H3: source-matched taxonomic depth

H3 treated taxonomic composition as a biological level of response rather than a nuisance correction. Outcome-blind source expectations were constructed from fixed source-flora assignments. The broad response was evaluated at three stages: observed assemblage response, residual after family composition, and residual after source-matched genus composition. The key estimand was the attenuation in response-vector magnitude between these stages, together with the predeclared support status at each depth.

A loss of support after genus adjustment was interpreted as evidence that the broad response is principally represented by genus-level assembly. It was not interpreted as proof that dispersal alone caused the filter, proof that within-genus response is absent, or proof that evolution did not occur.
...
Tropical assemblages did not follow a weaker copy of the Palearctic branch. In direct-only all-native data, accessibility/generalization declined with distance (-0.1005, q=0.00275) while reproductive assurance increased (0.1360, q=0.00428). Thus increasing reproductive assurance did not imply increasing floral generalization. The two response components can be decoupled at macroecological scale.

The Palearctic attraction/access component also remained positive after conditioning on `selfing_core` across four fixed source definitions. In all-native assemblages, conditional distance estimates were approximately 0.091–0.101 with q=0.0079–0.034. This further argues against a compulsory serial model in which isolation first increases selfing and floral architecture then changes only as a secondary selfing syndrome.

Secondary large-bee-like, butterfly-like, and bird-like templates differed among regions, but a source-trained common factor explained 86.94% of their variance in all-analysis evidence and 86.44% in direct-only evidence. These named templates therefore mainly captured overlapping plant architecture. They were not treated as evidence for realized visitor identity.

### H3: the strongest Palearctic syndrome concentrated at the family-to-genus assembly transition

Taxonomic-depth decomposition supplied the strongest explanatory result. Across the four combinations of evidence scope and floristic stratum, the broad Palearctic response passed the predeclared gate at the observed assemblage stage, remained supported after family adjustment, and failed after source-matched genus adjustment: `4/4 -> 4/4 -> 0/4`.

In vector-magnitude terms, family adjustment removed approximately 19.6–33.4% of the observed response. Genus adjustment removed approximately 78.8–85.9% relative to the observed response. Conditional on having already removed family composition, the transition to genus composition removed approximately 70.6–79.1% of the remaining signal. The attenuation was therefore not diffuse across taxonomy; most of it was concentrated between family and genus.

This result changes the biological interpretation of the syndrome. The strongest Palearctic gradient is not well described as a robust beyond-genus response repeated across the flora. It is principally expressed through which source-available genera are represented on islands along the separation gradient. That conclusion does not specify whether genus representation is generated by dispersal, habitat, establishment, persistence, interaction dependence, or combinations of these processes.

A separate hierarchical-depth audit showed that taxonomic attenuation itself differs among contexts within widespread native non-endemic assemblages. This supports the broader claim that biogeographic contingency is not limited to the sign of a trait coefficient; the **assembly depth** of the response can also differ.
...
When the observed outcomes were finally opened under those frozen gates, all 12 broad atomic response cells were classified as `monotonic_or_unresolved`; no step, hinge, or reversal was promoted. The global assemblage evidence therefore does not support one common isolation breakpoint. This result does not rule out lineage-specific or local-system thresholds.
...
### From “why are island flowers dull?” to “where does the syndrome live?”

The original question behind this analysis was simple: why should island flowers become duller, more generalized, or more self-reliant with isolation? The data do not support answering that question with one adaptive story. Instead, they change the object of explanation. The first question is no longer *which mechanism makes island flowers dull?* but *at what biological level is the apparent syndrome expressed?*

Three results drive that shift. First, the response is not globally uniform: Palearctic and tropical assemblages can combine reproductive and floral changes differently. Second, the strongest Palearctic response is sharply attenuated at the family-to-genus transition. Third, several simple mechanistic stories fail explicit tests: continuous area does not promote, a common breakpoint is not recovered despite adequate step-detection ability, coarse pollinator-channel heterogeneity is not supported, sampled interaction breadth does not promote, and the independent biotic-versus-wind contrast is null in the only cell qualified for inspection.

The parsimonious description is therefore a **hierarchically assembled phenotypic syndrome**. The visible floral/reproductive gradient is an assemblage property whose components can be generated by changing community membership, changing traits within represented lineages, or both. In the strongest Palearctic branch, changing genus representation accounts for most of the broad primary response.

### Taxonomic depth is an estimand, not merely a correction

Community-level trait studies often control for taxonomy to remove non-independence. Here the attenuation trajectory itself is biologically informative. If a broad isolation response survives family adjustment but largely disappears after genus adjustment, the family-to-genus transition identifies the scale at which source-pool sorting is most strongly represented. Taxonomic depth therefore becomes an estimand: **where in the lineage hierarchy does a macroecological syndrome live?**

This framing avoids two opposite errors. One is to interpret every assemblage-level trait gradient as repeated adaptation. The other is to treat taxonomy as an inconvenient confounder that should simply be regressed away. Lineage composition is itself a potential outcome of geographic filtering. A genus-level signal can reflect propagule movement, habitat compatibility, establishment, persistence, mutualist dependence, or historical source structure. The correct inference is not that “taxonomy explains the biology,” but that the biology is expressed substantially through lineage assembly at that depth.
...
This distinction protects the main result from overreach. Chapter 1 identifies **where the syndrome is expressed** and which simple explanations fail. It does not infer an unmeasured historical interaction mechanism merely because the traits are floral.
...
### From island floral syndrome to a general theory of assembled trait syndromes

The broader implication extends beyond islands. Macroecology frequently describes repeated “syndromes” along urbanization, elevation, aridity, fragmentation, disturbance, or other environmental gradients. Such syndromes are often narrated as repeated adaptive responses of organisms. But an assemblage mean can change because organisms change, because community membership changes, or because both processes occur at different taxonomic depths.

Our results suggest a general diagnostic sequence for any cross-sectional trait syndrome. Ask: **Which components move? In what direction? Does the response branch among contexts? At what taxonomic depth does it attenuate? Is the apparent geometry identifiable? Which observation and process alternatives survive explicit falsification?** Only after those questions are answered should a named mechanism be promoted.

In this view, the novelty of the island case is not simply that some flowers become more generalized or more reproductively assured. It is the demonstration that the same geographic gradient can reorganize trait components differently across regions and that the strongest broad branch can be localized to a specific taxonomic assembly transition. The syndrome is therefore not merely a trait list; it has an **assembly depth**.

### Chapter 1 and Chapter 2 resolve different scales of the same problem

The global analysis mixes many lineages and many island histories. Its strength is breadth: it can identify component direction, biogeographic branching, taxonomic attenuation, and failures of universal explanations. Its weakness is mechanism: it cannot reconstruct realized interaction service for each lineage.
...
The two scales are therefore complementary. **Chapter 1 asks where and at what assembly level the syndrome appears. Chapter 2 asks how a local interaction–reproduction–phenotype chain generates a particular response geometry.** A local threshold and a smooth global assemblage gradient are logically compatible, but the present study demonstrates that the global data alone cannot identify a distributed-threshold generator against heterogeneous smooth alternatives.

### Conclusion

Geographic isolation does not impose one floral island syndrome. It reorganizes floral and reproductive composition in ways that depend on biogeographic context, and the strongest Palearctic response is concentrated at the family-to-genus assembly transition. That result survives substantial trait-missingness and species-detection challenges, while several simpler explanations fail prospective tests. At the same time, the global data do not identify a pollinator-specific causal mechanism and do not justify interpreting a smooth assemblage gradient as evidence for or against local thresholds.

The island-flower question therefore leads to a broader inference problem: **when does a macroecological trait syndrome represent repeated adaptation of organisms, and when does it emerge from which lineages can assemble?** Our results show that identifying the assembly depth of a syndrome is a necessary step before assigning mechanism to it.

---

## Figure 1 narrative anchor

Figure 1 should begin with the island question rather than with a generic macroecological theory. The visual sequence is:

1. **Original observation:** “Why are island flowers dull?” with three floral/reproductive components shown separately rather than as one icon.
2. **Rival generators:** repeated within-lineage response versus hierarchical lineage sorting, with a mixed pathway allowed.
3. **Sequential test:** universal response -> context branching -> family/genus attenuation -> area/geometry falsification -> independent mechanism gate.
4. **Scale bridge:** many-lineage Chapter 1 macroecology on the left and one-system Chapter 2 interaction/service/reproduction/phenotype chain on the right.

The figure must visually mark the evidence boundary: plant-side assembly depth is directly supported; pollinator-specific causation is not.

---

## Claim ceiling for v8

The manuscript may state that the strongest Palearctic floral/reproductive syndrome is principally expressed through source-matched genus-level assembly and that trait-syndrome direction and assembly depth depend on biogeographic context. It may state that several simple alternatives fail explicit falsification tests.

It must not state that pollinator loss caused the Palearctic response, that genus attenuation proves dispersal alone, that the H5c null proves pollinators do not matter, that tropical accessibility is as observation-robust as the Palearctic branch, that endemicity establishes within-lineage evolution, or that smooth global responses imply the absence of local thresholds.
