...
```text
105,612 scoped angiosperm species
...
The 105,612 denominator is produced by the same taxonomic scope and excluded-
family rules as `global_trait_campaign.py`. A denominator or master fingerprint
change fails closed instead of silently moving species between shards.
...
The production cache uses contract v4; pre-tuning pilot caches are intentionally
ignored so completed rows are regenerated with the corrected context, cultivar,
and syndrome rules.
