...
> **2026-08-12 追記 — 実測による訂正**
>
> 本書を書いた後にコミット済み coverage ledger を実測し、
> [`docs/unresolved_trait_cause_analysis.md`](unresolved_trait_cause_analysis.md)
> にまとめた。その結果、本書の 2 箇所が誤りであることが判明した。
>
> 1. **非被子植物は既に分母から除かれている。** strict denominator
>    `106,295` は flora master `115,328` から 9,033 行を既に除外済みで、
>    非被子植物の未解決 cells は **0** である。2.3 の
>    「non-angiosperm 838種を未解決から外す」は strict 分母には適用されない。
> 2. **上位10,000種は既に 58.7% 解決済み**であり、未解決 backlog の
>    5.1% を占めるにすぎない。「上位1万種を取りにいく」という第1期設計は
>    粗すぎる。実測が示す律速は種ではなく**軸**であり、
>    花色は既に終わっていて、構造と繁殖保証だけが残っている。
>
> 島重み付けという基本方針（分母を flora mass 側へ寄せる）は実測でも支持されるが、
> 実行単位は「上位N種」ではなく「軸 × あと1軸で ready になる島」である。
> 具体的な標的リストは原因分析の 5 節にある。
>
> **2026-08-16 追記 — 上位1万種戦略の撤回**
>
> [`docs/coverage_confound_analysis.md`](coverage_confound_analysis.md) の実測により、
> 本書の中心的な処方（上位1万種の広域種を取得する）は**撤回する**。
> 被覆は島の固有種率と負に相関しており（Spearman rho = −0.369、
> 低固有種率の島で 71.5%、高固有種率の島で 28.0%）、
> 広域種は既に被覆の高い島に偏在している。
> したがって上位1万種を埋めるほど交絡は急峻になる。
>
> 本問題は取得計画の調整では解決しない。欠測モデルまたは共通サポート制限という
> 設計側の対応が必要である。詳細は上記文書の 5 節。
