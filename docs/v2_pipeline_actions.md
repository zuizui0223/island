...
| 0.5 Bombus applicability freeze | `island-v2-bombus-applicability build` | manual, before outcome traits are inspected | Join reviewed source-region registry, evidence, and island-to-source assignments → every frozen island is explicitly `applicable`, `structurally_not_applicable`, or `unresolved`, plus Core/contingency/out-of-domain tables. Climate, Bombus records, floral traits, reproductive traits, and model results are rejected as inputs. |
| 0.6 Core-pilot nomination | `island-v2-core-pilot-nomination nominate` | manual, after 0.5 and after collection | Nominate Core-pilot islands from **only** frozen applicability (`applicable` & not `regional_analysis_only`) **and** whether the current collection produced ≥ N exact-island flora species. Never uses traits, Bombus records, or model results. When none are eligible it reports the gap (which applicable islands await flora collection) instead of inventing a Core island. A nominee still needs a flora anchor and taxon/establishment review before any analysis table. |
