...
def get_json(url: str) -> dict:
    req=Request(url,headers={'User-Agent':'island-wave58/1.1 (https://github.com/zuizui0223/island)'})
...
def main() -> None:
    ...
    summary={'contract':'wave58_exact_species_reproductive_discovery_v2','validated_support2_genus_trait_pairs':len(pairs),'target_species':len(targets),'workers':a.workers,'acquisition_executed':a.acquire,'query_success':sum(x['status']=='success' for x in logs),'query_errors':sum(x['status']=='error' for x in logs),'unreviewed_document_leads':len(rows),'lead_species':len({x['accepted_species'] for x in rows}),'automatic_promotions':0,'canonical_net_gain_verified':None}
