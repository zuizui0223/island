...
BLOCKED={'Sideroxylon','Illicium','Portulaca','Eugenia','Liparis','Melaleuca','Callicarpa','Durio','Ourisia','Schoenoplectiella','Spermacoce','Dicliptera','Gomphrena','Marcgravia','Melicytus','Onopordum','Pouteria','Lindernia'}


def get_json(url: str) -> dict:
    req=Request(url,headers={'User-Agent':'island-wave58/1.4 (https://github.com/zuizui0223/island)'})
...
def main() -> None:
    ...
    unresolved=cov[(cov.axis.eq(AXIS))&(cov.quality.eq(''))].copy(); unresolved=unresolved[unresolved.accepted_species.map(lambda x: bool(BINOMIAL.fullmatch(x)))].copy(); unresolved['genus']=unresolved.accepted_species.str.split().str[0]; counts=unresolved.groupby('genus').size()
    ...
    targets=(targets.sort_values(['genus','accepted_species']).groupby('genus',as_index=False,group_keys=False).head(8).sort_values(['leverage','genus','accepted_species'],ascending=[False,True,True]).head(a.max_species)); assert targets.accepted_species.map(lambda x: bool(BINOMIAL.fullmatch(x))).all(); targets.to_csv(a.out/'exact_species_targets.csv',index=False)
    rows=[]; logs=[]
    def one(species: str):
