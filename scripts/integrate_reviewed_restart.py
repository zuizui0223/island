...
def integrate(c, d, records, ontology):
    ...
    for r in records:
        ...
        if trait['domain'] != r['axis'] or r['normalized_value'] not in trait['allowed_values'] or r['normalized_value'] == 'unresolved':
        ...
        if r['cultivar_status'] != 'natural_populations' or r['genus_rule_training_allowed']:
            raise ValueError('Unsupported promotion scope')
        mask = c.accepted_species.eq(key[0]) & c.axis.eq(r['axis'])
        if mask.sum() != 1 or c.loc[mask, 'quality'].iloc[0] != '':
            raise ValueError('Not an empty baseline axis; requires full conflict resolution')
        state = json.dumps([r['normalized_value']], separators=(',', ':'))
        c.loc[mask, 'quality'] = r['quality']
        c.loc[mask, 'trait_names'] = r['trait_name']
        c.loc[mask, 'trait_composition'] = r['trait_name'] + '=' + state
        c.loc[mask, 'source_groups'] = 'reviewed_restart_20260907'
        c.loc[mask, 'source_lineages'] = r['source_lineage']
        ...
        row.update(accepted_species=key[0], axis=r['axis'], trait_name=key[1],
            ...
            state_sets=json.dumps([[r['normalized_value']]]), normalized_value=r['normalized_value'],
            n_independent_lineages='1', source_lineages=r['source_lineage'],
            source_groups='reviewed_restart_20260907', multistate='False', genus=key[0].split()[0])
...
def main():
    ...
    shutil.copy2(a.records, a.output/'reviewed_source_records.json')
    ...
    summary = dict(baseline_run=34093932899, baseline_artifact=10007872352,
        ...
        input_sha256={str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [cf,df,a.records,a.ontology]},
