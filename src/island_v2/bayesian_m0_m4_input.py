...
computed within animal-pollinated species (pollen_vector_mode in {biotic, mixed}), matching
v1's biological scope while retaining v2's finer categories for later multinomial models.
"""
...
def _count_contrast(
    ...
    *,
    ...
    x = data.loc[data[column].astype(str).isin(denominator), ["island_id", "accepted_species", column]].copy()
    x = x.drop_duplicates(["island_id", "accepted_species"])
...
def _guild_counts(data: pd.DataFrame) -> pd.DataFrame:
    x = data.loc[data["pollination_functional_guild"].notna(), ["island_id", "accepted_species", "pollination_functional_guild"]].copy()
    x = x.drop_duplicates(["island_id", "accepted_species"])
    x["showy_alt"] = x["pollination_functional_guild"].astype(str).isin(SHOWY_ALT_GUILDS).astype(int)
    x["any_alt"] = x["pollination_functional_guild"].astype(str).isin(ALL_ALT_GUILDS).astype(int)
    out = x.groupby("island_id").agg(
...
def build(master_csv: Path, island_covariates_csv: Path, output_dir: Path) -> None:
    ...
    animal = species.loc[species["pollen_vector_mode"].astype(str).isin(ANIMAL_VECTOR_VALUES)].copy()

    tables = [
        _count_contrast(
            animal,
            column="flower_primary_color",
            positive=PLAIN_COLORS,
            denominator=PLAIN_COLORS | CONSPICUOUS_COLORS,
            prefix="plain_color",
        ),
        _count_contrast(
            animal,
            column="floral_form",
            positive=GENERALIZED_FORMS,
            denominator=GENERALIZED_FORMS | SPECIALIZED_FORMS,
    ...
    # Continuous interaction terms are standardized in R after complete-case filtering.
    analysis["log_distance"] = pd.to_numeric(analysis["log_distance_to_continent_km"], errors="coerce")
    analysis["log_area"] = pd.to_numeric(analysis["log_island_area_km2"], errors="coerce")
    analysis["bombus_compatibility"] = pd.to_numeric(analysis["bombus_channel_state"], errors="coerce")
    analysis["is_northern_midlatitude"] = analysis["analysis_regime"].astype(str).eq("northern_midlatitude")
    ...
    category_rows = []
    for trait_name in ("flower_primary_color", "floral_form", "floral_symmetry", "pollination_functional_guild"):
        if trait_name not in animal.columns:
            continue
        x = animal.loc[animal[trait_name].notna(), ["island_id", "accepted_species", trait_name]].drop_duplicates()
        x = x.groupby(["island_id", trait_name]).size().rename("n_species").reset_index().rename(columns={trait_name: "category"})
        x.insert(1, "trait_name", trait_name)
        category_rows.append(x)
    pd.concat(category_rows, ignore_index=True).to_csv(output_dir / "bayesian_m0_m4_category_counts.csv", index=False)
