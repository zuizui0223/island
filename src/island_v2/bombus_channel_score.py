...
def aggregate_environmental_compatibility(species_scores: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise typer.BadParameter(f"environmental score table missing columns: {sorted(missing)}")

    work = species_scores.copy()
    work["environmental_compatibility"] = pd.to_numeric(
        work["environmental_compatibility"], errors="coerce"
    )
    work = work.dropna(subset=["island_id", "bombus_species", "environmental_compatibility"])
    ...
    for island_id, group in work.groupby("island_id", sort=True):
        ...
        rows.append(
            {
                "island_id": island_id,
                "n_source_pool_bombus_species": int(group["bombus_species"].nunique()),
...
def score_observation_evidence(
    diagnostics: pd.DataFrame, config: dict[str, Any]
) -> pd.DataFrame:
    ...
    count_scale = float(config.get("continuous_score", {}).get("bombus_record_scale", 3.0))
    max_age = float(recency["max_years_since_latest_background"])

    rows: list[dict[str, Any]] = []
    for _, row in diagnostics.iterrows():
        bombus_count = float(pd.to_numeric(pd.Series([row.get("bombus_record_count", 0)]), errors="coerce").fillna(0).iloc[0])
        occurrence_support = 1.0 - math.exp(-bombus_count / max(count_scale, 1e-9))

        effort_parts = [
            _ratio(row.get("target_group_record_count", 0), float(thresholds["min_background_records"])),
            _ratio(row.get("target_group_spatial_units", 0), float(thresholds["min_background_spatial_units"])),
            _ratio(row.get("target_group_temporal_units", 0), float(thresholds["min_background_temporal_units"])),
            _ratio(row.get("distinct_dataset_count", 0), float(thresholds["min_distinct_datasets"])),
        ]
        structural_effort = math.prod(effort_parts) ** (1.0 / len(effort_parts))

        age = pd.to_numeric(pd.Series([row.get("latest_record_age_years", pd.NA)]), errors="coerce").iloc[0]
        ...
        observation_availability = 0.5 + effort_quality * (occurrence_support - 0.5)
        ...
        rows.append(
            {
                ...
                "bombus_occurrence_evidence": row.get("bombus_occurrence_evidence", ""),
            }
        )
    return pd.DataFrame(rows)
...
def combine_channel_components(
    observation: pd.DataFrame, environmental: pd.DataFrame | None = None
) -> pd.DataFrame:
...
def run(
    ...
    if environmental_scores_csv is not None:
        species_scores = pd.read_csv(environmental_scores_csv, dtype=str).fillna("")
    ...
    observation.to_csv(output_dir / "bombus_observation_continuous_scores.csv", index=False)
    if environmental is not None:
        environmental.to_csv(output_dir / "bombus_environmental_compatibility.csv", index=False)
    result.to_csv(output_dir / "bombus_channel_continuous_scores.csv", index=False)
    summary = {
        "n_islands": int(len(result)),
        "component_policy": "keep environmental compatibility and observation evidence separate; combined score is secondary",
        "combined_formula": "sqrt(environmental_compatibility * observation_availability)",
        "observation_formula": "0.5 + effort_quality * (occurrence_support - 0.5)",
    }
