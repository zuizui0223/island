...
def load_inputs(v6_root: Path, geometry_root: Path, h5c_root: Path, h5d_root: Path) -> dict[str, Any]:
    ...
    geom_required = {"all_observed_D", "all_critical_D", "direct_observed_D", "direct_critical_D", "classification"}
    h5d_required = {"classification_accuracy", "false_distributed_selection_under_smooth", "qualified"}
    for frame, required, label in ((v6, v6_required, "V6"), (geom, geom_required, "geometry"), (h5d, h5d_required, "H5d")):
        missing = required - set(frame.columns)
        if missing:
            raise FigureInputError(f"{label} missing columns: {sorted(missing)}")

    expected_targets = {"Palearctic_accessibility", "tropical_accessibility", "north_tropical_vector_difference"}
    if set(v6["target"].astype(str)) != expected_targets:
        raise FigureInputError("V6 target set differs from frozen contract")
    if len(geom) != 12 or set(geom["classification"].astype(str)) != {"monotonic_or_unresolved"}:
        raise FigureInputError("geometry artifact must contain 12 monotonic_or_unresolved cross-scope cells")
...
def _panel_v6(ax: plt.Axes, v6: pd.DataFrame) -> dict[str, tuple[int, int]]:
    labels = ["Palearctic accessibility", "North–Tropical vector", "Tropical accessibility"]
    targets = ["Palearctic_accessibility", "north_tropical_vector_difference", "tropical_accessibility"]
    summary: dict[str, tuple[int, int]] = {}
    survive = []
    for target in targets:
        sub = v6.loc[v6["target"].astype(str).eq(target) & v6["baseline_supported"].astype(bool)]
        total = len(sub)
        kept = int(sub["tipping_event"].astype(str).eq("none_on_frozen_grid").sum())
    ...
    ax.set_title("A  Observation-bias defense is asymmetric", loc="left", fontsize=10.5, fontweight="bold")
    for i, (target, frac) in enumerate(zip(targets, survive, strict=True)):
        kept, total = summary[target]
        ax.text(min(frac + 0.02, 0.90), i, f"{kept}/{total}", va="center", fontsize=8.5, fontweight="bold")
...
def _panel_geometry(ax: plt.Axes, geom: pd.DataFrame) -> None:
    ...
    y = pd.to_numeric(geom["direct_observed_D"]) - pd.to_numeric(geom["direct_critical_D"])
    ...
    ax.set_title("B  A nonlinear shape required both scopes", loc="left", fontsize=10.5, fontweight="bold")
    ax.text(0.98, 0.96, "promotion quadrant", transform=ax.transAxes, ha="right", va="top", fontsize=7.7, color="0.35")
    ax.text(0.03, 0.05, "0/12 promoted", transform=ax.transAxes, fontsize=9, fontweight="bold")
...
def _panel_h5c(ax: plt.Axes, result: dict[str, Any]) -> None:
    ...
    ax.axvline(0, color="0.55", linestyle="--", linewidth=0.9)
    ax.errorbar([est], [0], xerr=[[est - lo], [hi - est]], fmt="o", capsize=3, markersize=6)
    ax.set_yticks([0])
    ax.set_yticklabels(["distance × biotic"], fontsize=8.5)
    ax.set_ylim(-0.8, 0.8)
    ax.set_xlabel("Interaction estimate", fontsize=8.5)
    ax.set_title("C  Independent pollination-mode specificity", loc="left", fontsize=10.5, fontweight="bold")
    ax.text(0.03, 0.12, f"p = {float(result['interaction_p_value']):.3f}\nnot promoted", transform=ax.transAxes, fontsize=8.5, fontweight="bold")
    for spine in ("top", "right", "left"):
        ax.spines[spine].set_visible(False)
    ax.tick_params(axis="x", labelsize=8)
...
def _panel_h5d(ax: plt.Axes, h5d: pd.DataFrame) -> None:
    ...
    ax.set_title("D  Distributed thresholds were not identifiable", loc="left", fontsize=10.5, fontweight="bold")
    ax.text(0.03, 0.94, "required: accuracy ≥0.80\nand false selection ≤0.10", transform=ax.transAxes, va="top", fontsize=7.7, color="0.35")
    ax.text(0.97, 0.06, "0/8 qualified", transform=ax.transAxes, ha="right", fontsize=9, fontweight="bold")
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    ax.tick_params(labelsize=7.8)


def render_figure(*, v6_root: Path, geometry_root: Path, h5c_root: Path, h5d_root: Path, output_dir: Path, source_receipts: dict[str, Any]) -> dict[str, Any]:
    ...
    grid = GridSpec(2, 2, figure=fig, left=0.09, right=0.97, bottom=0.10, top=0.88, hspace=0.40, wspace=0.34)
    ax_a = fig.add_subplot(grid[0, 0]); ax_b = fig.add_subplot(grid[0, 1]); ax_c = fig.add_subplot(grid[1, 0]); ax_d = fig.add_subplot(grid[1, 1])
    ...
    fig.suptitle("Cross-examination defines what the global data can and cannot support", x=0.09, y=0.975, ha="left", fontsize=14, fontweight="bold")
    fig.text(0.09, 0.944, "Frozen falsification and identifiability audits; adverse evidence is shown rather than rescued.", fontsize=9, color="0.35")
    basename = "chapter1_v8_figure4_falsification_boundaries"
    png = output_dir / f"{basename}.png"; svg = output_dir / f"{basename}.svg"; pdf = output_dir / f"{basename}.pdf"
    fig.savefig(png, dpi=300, bbox_inches="tight"); fig.savefig(svg, bbox_inches="tight"); fig.savefig(pdf, bbox_inches="tight"); plt.close(fig)
    manifest = {
        "contract": "chapter1_v8_figure4_falsification_boundaries_v1",
        "status": "rendered_from_four_frozen_audits",
        "sources": source_receipts,
        "v6_survival": {key: {"survived": value[0], "baseline_supported": value[1]} for key, value in v6_summary.items()},
        "geometry_promoted": 0,
        ...
        "h5d_qualified": int(pd.Series(tables["h5d"]["qualified"]).astype(bool).sum()),
        "h5d_cells": int(len(tables["h5d"])),
        "new_biological_models_fitted": False,
        "new_p_values_generated": False,
        "claim_boundary": "Figure 4 visualizes frozen stress tests and failed/non-identified mechanism gates; it does not promote a mechanism from negative evidence.",
        "outputs": [png.name, svg.name, pdf.name],
    }
    (output_dir / "chapter1_v8_figure4_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    return manifest
...
def render_command(
    ...
    typer.echo(json.dumps(render_figure(v6_root=v6_root, geometry_root=geometry_root, h5c_root=h5c_root, h5d_root=h5d_root, output_dir=output_dir, source_receipts=receipts), indent=2))
