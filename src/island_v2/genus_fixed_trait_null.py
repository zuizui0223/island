"""Genus-composition-preserving null model for island floral traits.

This is a lineage sensitivity, not missing-data imputation. Direct species-level
trait states are permuted among species within the same genus. Species geographic
occurrence and each island's observed genus composition stay fixed. A single
permuted species state is shared across every island occurrence of that species in
a draw, avoiding island-row pseudoreplication.
"""
...
def classify_binary_value(value: object, positive: set[str], negative: set[str]) -> float:
    """Classify a multistate value only when every state agrees on one side."""
...
def direct_binary_species(
    ...
    positive = {str(value) for value in spec["positive_states"]}
    negative = {str(value) for value in spec["negative_states"]}
    ...
    frame["state"] = [
        classify_binary_value(value, positive, negative) for value in frame["normalized_value"]
    ]
    frame = frame.dropna(subset=["state"]).drop_duplicates("accepted_species")
    ...
    frame = frame.merge(taxa, on="accepted_species", how="left", validate="one_to_one")
    return frame.loc[frame["genus"].ne("")].reset_index(drop=True)


def _permuted_state_map(
    species_states: pd.DataFrame, rng: np.random.Generator
) -> pd.Series:
    """Permute states among species within genus and return species-indexed state."""
...
def _aggregate(
    ...
    out = (
        ...
        .agg(n_species=("state", "size"), successes=("state", "sum"))
    )
    ...
    for outcome, spec in binary_outcomes.items():
        ...
        for stratum in STRATA:
            ...
            observed = observed.loc[
                observed["observed_n_species"] >= min_species_per_island
            ].copy()
            if observed.empty:
                continue
            ...
            for draw in range(1, n_draws + 1):
                ...
                agg = agg.loc[
                    agg["island_id"].isin(observed["island_id"]), ["island_id", "share"]
                ]
                agg["draw"] = draw
                draws.append(agg)
            draw_table = pd.concat(draws, ignore_index=True)
            null = (
                draw_table.groupby("island_id", as_index=False)
                .agg(
            ...
            merged["deviation_observed_minus_null"] = (
                merged["observed_share"] - merged["null_mean"]
            )
            merged["ses"] = merged["deviation_observed_minus_null"] / merged["null_sd"].replace(
                0, np.nan
            )
...
def run(
    ...
    min_species_per_island: int = typer.Option(30, min=1),
) -> None:
    ...
    manifest = {
        "contract": "genus_fixed_trait_null_v1",
        "n_draws": n_draws,
        "seed": seed,
        "min_species_per_island": min_species_per_island,
        "n_result_rows": int(len(results)),
        "interpretation": (
            "Lineage-composition sensitivity only. States are permuted among species "
            "within genus; this does not fill missing traits or create direct evidence."
        ),
    }
