...
def apply_direct_evidence_exclusions(
    ...
    with a lineage-prefix key ending in ``*``.  This deliberately narrow
    wildcard form lets one reviewed ontology decision remove every affected
    redistribution row without pretending that each species was independently
    reviewed.
    """
    ...
    wildcard = work[_EXCLUSION_KEY].apply(
        lambda column: column.astype(str).str.contains("*", regex=False)
    ).any(axis=1)
    valid_wildcard = (
