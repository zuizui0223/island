...
COVERAGE_COLUMNS = ["accepted_species", *[column for axis in AXES for column in (f"{axis}_direct", f"{axis}_genus_inferred", f"{axis}_resolved")], "all_three_axes_direct", "all_three_axes_resolved", "unresolved_axes"]
...
def _read_many(paths: list[Path]) -> pd.DataFrame:
    frames = [frame for frame in (_read(path) for path in paths) if not frame.empty]
...
def _quality(provider: str, title: str, scope: str) -> str:
    if scope == "genus_inference":
        return "low"
    return "high" if any(marker in f"{provider} {title}".casefold() for marker in HIGH_MARKERS) else "medium"
...
def _normalise(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    aliases = {
        "species": "accepted_species", "matched_name": "matched_source_name", "source_name": "matched_source_name",
        "trait": "trait_name", "field": "trait_name", "candidate_value": "raw_candidate_value",
        "provisional_candidate_value": "raw_candidate_value", "value": "reported_value",
        "raw_description": "excerpt", "source_excerpt": "excerpt", "source_text": "excerpt",
        "source_citation": "source_title", "source_type": "provider", "source_kind": "provider",
        "doi": "stable_source_id", "source_id": "stable_source_id", "fill_tier": "evidence_scope",
        "evidence_type": "evidence_scope",
    }
    frame = frame.rename(columns={old: new for old, new in aliases.items() if old in frame.columns and new not in frame.columns}).copy()
    ...
    frame["matched_source_name"] = frame["matched_source_name"].where(frame["matched_source_name"].ne(""), frame["accepted_species"])
    frame["evidence_scope"] = frame["evidence_scope"].replace({"": "species_direct", "direct": "species_direct", "flora": "species_direct", "reported": "species_direct", "inference": "genus_inference"})
    frame = frame.loc[~frame["evidence_scope"].isin(PROHIBITED_SCOPES)].copy()
    frame["evidence_quality"] = [_quality(provider, title, scope) for provider, title, scope in zip(frame["provider"], frame["source_title"], frame["evidence_scope"], strict=False)]
    ...
    frame = frame.loc[frame["accepted_species"].ne("") & frame["trait_name"].ne("") & ~value.str.casefold().isin(EMPTY) & (valid_direct | frame["evidence_scope"].eq("genus_inference"))].copy()
    return frame[LEDGER_COLUMNS].drop_duplicates(subset=["accepted_species", "matched_source_name", "trait_name", "reported_value", "raw_candidate_value", "source_url", "excerpt", "evidence_scope"])


def _empty_shard_status(campaign_root: Path) -> bool:
    files = sorted(campaign_root.rglob("campaign_status.json"))
    if not files:
        return False
    try:
        status = json.loads(files[0].read_text(encoding="utf-8"))
...
def aggregate(campaign_root: Path, output_dir: Path) -> dict[str, object]:
    ...
    if empty_shard:
        ...
        processed_species = sorted({_text(value) for value in checkpoint.loc[attempted, "species"] if _text(value)})
    ...
    if not provider.empty and {"provider", "status", "species"}.issubset(provider.columns):
        ...
        for name, group in active.groupby("provider"):
            ...
            provider_state[_text(name)] = {
                "hit": int(statuses.eq("hit").sum()), "no_hit": int(statuses.eq("no_hit").sum()),
    ...
    quality_counts = evidence["evidence_quality"].value_counts().reindex(["high", "medium", "low"], fill_value=0).astype(int).to_dict()
    direct_coverage = {axis: int(coverage[f"{axis}_direct"].sum()) if not coverage.empty else 0 for axis in AXES}
    summary = {
        "contract_version": "real_acquisition_ledger_v1",
        "evidence_hierarchy": {"high": "literature_or_database", "medium": "species_or_synonym_web_description", "low": "genus_inference"},
        "family_inference_used": False, "global_fallback_used": False, "empty_shard": empty_shard,
        "total_species_in_shard": len(all_species), "total_species_processed": len(processed_species),
        "species_with_any_evidence": int(evidence["accepted_species"].nunique()), "evidence_records": int(len(evidence)),
        "evidence_records_by_quality": quality_counts, "direct_coverage": direct_coverage,
        "species_with_all_three_axes_direct": int(coverage["all_three_axes_direct"].sum()) if not coverage.empty else 0,
        "species_with_all_three_axes_resolved": int(coverage["all_three_axes_resolved"].sum()) if not coverage.empty else 0,
        "unresolved_species_count": int(coverage["unresolved_axes"].ne("").sum()) if not coverage.empty else 0,
    ...
    (output_dir / "collection_round_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return summary


@app.command()
def build(campaign_root: Path = typer.Option(..., exists=True, file_okay=False), output_dir: Path = typer.Option(...)) -> None:
