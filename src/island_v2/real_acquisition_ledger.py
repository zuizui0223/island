...
def _normalise(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["evidence_quality"] = ["low" if scope == "genus_inference" else ("high" if any(marker in f"{provider} {title}".casefold() for marker in HIGH_MARKERS) else "medium") for provider, title, scope in zip(frame["provider"], frame["source_title"], frame["evidence_scope"], strict=False)]
    ...
    frame = frame.loc[frame["accepted_species"].ne("") & frame["trait_name"].ne("") & ~value.str.casefold().isin(EMPTY) & (valid_direct | valid_low)].copy()
...
def aggregate(campaign_root: Path, output_dir: Path) -> dict[str, object]:
    ...
    summary = {
        ...
        "evidence_records_by_quality": evidence["evidence_quality"].value_counts().reindex(["high", "medium", "low"], fill_value=0).astype(int).to_dict(),
    }
