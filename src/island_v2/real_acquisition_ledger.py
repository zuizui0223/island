"""Aggregate actual public-web acquisition outputs into a three-axis evidence ledger.
...
HIGH_MARKERS = {
    "crossref", "doi", "europe_pmc", "pubmed", "openalex", "semantic_scholar",
    "journal", "literature", "flora", "monograph", "efloras", "gbif", "traitbank",
    "austraits", "gift", "usda", "database",
}
TRAIT_ALIASES = {
...
LEDGER_COLUMNS = [
    "accepted_species", "matched_source_name", "trait_name", "reported_value",
    "raw_candidate_value", "source_url", "provider", "source_title", "excerpt",
    "retrieval_date", "stable_source_id", "evidence_scope", "evidence_quality",
]
...
def _normalise(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = frame.rename(
        columns={old: new for old, new in aliases.items() if old in frame.columns and new not in frame.columns}
    ).copy()
    ...
    frame["evidence_scope"] = frame["evidence_scope"].replace(
        {"": "species_direct", "direct": "species_direct", "flora": "species_direct",
         "reported": "species_direct", "inference": "genus_inference"}
    )
    ...
    return frame[LEDGER_COLUMNS].drop_duplicates(
        subset=[
            "accepted_species", "matched_source_name", "trait_name", "reported_value",
            "raw_candidate_value", "source_url", "excerpt", "evidence_scope",
        ]
    )
...
def aggregate(campaign_root: Path, output_dir: Path) -> dict[str, object]:
    ...
    if "attempts" in checkpoint.columns:
        attempted_mask |= pd.to_numeric(checkpoint["attempts"], errors="coerce").fillna(0).gt(0)
    if "status" in checkpoint.columns:
        attempted_mask |= ~checkpoint["status"].map(_text).isin({"", "pending"})
    processed_species = sorted({_text(value) for value in checkpoint.loc[attempted_mask, "species"] if _text(value)})
    ...
    quality_counts = evidence["evidence_quality"].value_counts().reindex(
