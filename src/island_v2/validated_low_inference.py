...
This module creates traceable low-quality evidence only when direct high/medium
species evidence shows strong within-genus consistency. Family inference and
unconditional fallback are intentionally excluded.
"""
...
def build_species_votes(evidence: pd.DataFrame) -> pd.DataFrame:
    """Collapse duplicate records to one vote per species/trait/value."""
    frame = _prepare(evidence)
    if frame.empty:
        return frame
    quality_rank = frame["evidence_quality"].map({"medium": 1, "high": 2}).fillna(0)
    frame = frame.assign(_quality_rank=quality_rank)
...
def build_genus_consensus(votes: pd.DataFrame, thresholds: Thresholds = Thresholds()) -> pd.DataFrame:
    ...
    for (genus, axis, trait), group in species_values.groupby(["genus", "axis", "trait_name"]):
        ...
        rows.append({
            ...
            "dominance": dominance,
            "required_dominance": _required_dominance(axis, thresholds),
            "eligible_before_validation": (
                n_species >= thresholds.min_species
                and dominance >= _required_dominance(axis, thresholds)
            ),
        })
    return pd.DataFrame(rows)
...
def masked_validate(votes: pd.DataFrame, consensus: pd.DataFrame) -> pd.DataFrame:
    """Leave-one-species-out validation for each genus/trait rule."""
    rows: list[dict[str, object]] = []
    ...
    for _, rule in consensus.iterrows():
        ...
        for species, truth in per_species.items():
            ...
            if training.empty:
                continue
            prediction = str(training.value_counts().index[0])
            outcomes.append(prediction == truth)
        rows.append({
            "genus": rule["genus"],
            "axis": rule["axis"],
...
def infer_low_evidence(
    ...
    rules["eligible"] = (
        rules["eligible_before_validation"]
        & rules["masked_accuracy"].ge(thresholds.min_masked_accuracy)
    )
    ...
    for axis, traits in AXES.items():
        ...
        merged = merged.loc[merged["trait_name"].isin(traits)].copy()
        ...
        accepted_parts.append(merged)

    accepted = pd.concat(accepted_parts, ignore_index=True) if accepted_parts else pd.DataFrame()
    resolved_keys = set(zip(accepted.get("accepted_species", []), accepted.get("axis", [])))
    remaining = tasks.loc[
...
def build(
    ...
    summary = {
        ...
        "resolved_species_axis_tasks": int(len(set(zip(accepted.get('accepted_species', []), accepted.get('axis', []))))),
        "remaining_tasks": int(len(remaining)),
        "eligible_rules": int(rules.get("eligible", pd.Series(dtype=bool)).sum()),
