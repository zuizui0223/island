"""Build the lossless Wave44 direct plus external-congener overlay."""
...
BASELINE_FORMAL_RUN_ID = 33_359_673_472
DIRECT_SOURCE_GROUPS = {
    "wave44_floraweb_biolflor_reproduction",
    "wave44_glopl_2025_reproduction",
}
...
def build_wave44_overlay(
    *,
    ...
    if missing := [str(path) for path in required if not path.is_file()]:
        raise ValueError(f"Wave44 overlay inputs missing: {missing}")
    ...
    if checkpoint["fixed_target_species"] != expected_species:
        raise ValueError("Wave44 checkpoint denominator mismatch")
    ...
    direct_mask = resolved["source_groups"].map(
        lambda value: bool(DIRECT_SOURCE_GROUPS.intersection(_split_pipe(value)))
    )
    direct = _validate_direct(resolved.loc[direct_mask].copy())
    touched = _touched_keys(reviewed_direct) | _touched_keys(external)

    key_columns = ["genus", "axis", "trait_name"]
    previous = previous_rules.loc[
        previous_rules["setting"].eq("current_min3")
        & _eligible(previous_rules)
    ].copy()
    current = rules.loc[
    ...
    for row in _aggregate_direct(direct).itertuples(index=False):
        ...
        if key not in result.index:
            raise ValueError(f"Wave44 direct evidence is outside the universe: {key}")
    ...
    for row in _aggregate_low(low).itertuples(index=False):
        key = (row.accepted_species, row.axis)
        if key not in result.index:
            raise ValueError(f"Wave44 Low is outside the fixed universe: {key}")
    ...
    if loss or downgraded.any():
        raise ValueError(
            f"Wave44 is not lossless: loss={loss}, "
    ...
    outputs = {
        "wave44_species_axis_coverage.csv.gz": result,
        "wave44_resolved_direct_species_trait.csv.gz": direct,
        "wave44_new_validated_low_species_trait.csv.gz": low,
        "wave44_new_trait_specific_genus_rule_audit.csv.gz": new_rules,
        "wave44_change_audit.csv.gz": changes_frame,
        "wave44_external_congener_resolved_species_trait.csv.gz": external_cells,
        "wave44_external_congener_source_conflicts.csv.gz": external_conflicts,
    }
    ...
    summary: dict[str, Any] = {
        "contract": "wave44_direct_external_reproduction_lossless_overlay_v1",
        ...
        "delta": {
            ...
            "resolved_wave44_direct_species_trait": len(direct),
            "resolved_wave44_direct_species_axis": int(
                direct[["accepted_species", "axis"]].drop_duplicates().shape[0]
            ),
            "new_external_input_species_trait": checkpoint["evidence"][
                "external_species_trait"
            ],
    ...
    (output_dir / "wave44_coverage_summary.json").write_text(
