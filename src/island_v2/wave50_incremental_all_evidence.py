"""Rebuild only Wave50-touched trait-specific genus rules."""
...
EXPECTED_NEW_RULES = frozenset(
    {
        ("Miscanthus", "reproductive_assurance", "self_incompatibility"),
        (
            "Pultenaea",
...
EXPECTED_COUNTEREXAMPLE_RULES = frozenset(
    {
        ("Polystachya", "reproductive_assurance", "autonomous_selfing_capacity"),
        ("Portulaca", "reproductive_assurance", "self_incompatibility"),
    }
)


def build_wave50_incremental_audit(
    *,
    master_csv: Path,
    ontology_yaml: Path,
    ...
    return build_incremental_audit(
        ...
        expected_direct_rows=5,
        expected_external_rows=0,
        expected_new_rules=EXPECTED_NEW_RULES,
        expected_blocked_rules=frozenset(),
        expected_counterexample_rules=EXPECTED_COUNTEREXAMPLE_RULES,
        output_label="wave50",
        contract="wave50_incremental_all_evidence_touched_rule_rebuild_v1",
    )
...
def main() -> None:
    ...
    summary = build_wave50_incremental_audit(**vars(args))
