"""Rebuild only Wave51-touched trait-specific Peperomia rules."""
...
EXPECTED_NEW_RULES = frozenset(
    {("Peperomia", "reproductive_assurance", "self_incompatibility")}
)
EXPECTED_SINGLE_LINEAGE_BLOCKED_RULES = frozenset(
    {("Peperomia", "reproductive_assurance", "autonomous_selfing_capacity")}
)


def build_wave51_incremental_audit(
    *,
    master_csv: Path,
    ontology_yaml: Path,
    ...
    return build_incremental_audit(
        ...
        expected_direct_rows=4,
        expected_external_rows=2,
        expected_new_rules=EXPECTED_NEW_RULES,
        expected_blocked_rules=EXPECTED_SINGLE_LINEAGE_BLOCKED_RULES,
        expected_counterexample_rules=frozenset(),
        output_label="wave51",
        contract="wave51_incremental_all_evidence_touched_rule_rebuild_v1",
    )
...
def main() -> None:
    ...
    summary = build_wave51_incremental_audit(**vars(args))
