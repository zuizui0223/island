"""Rebuild only Wave52-touched trait-specific reproductive rules."""
...
EXPECTED_NEW_RULES = frozenset(
    {
        ("Helichrysum", "reproductive_assurance", "self_incompatibility"),
        ("Mammillaria", "reproductive_assurance", "self_incompatibility"),
        ("Triumfetta", "reproductive_assurance", "self_incompatibility"),
    }
)


def build_wave52_incremental_audit(
    *,
    master_csv: Path,
    ontology_yaml: Path,
    ...
    return build_incremental_audit(
        ...
        output_label="wave52",
        contract="wave52_incremental_all_evidence_touched_rule_rebuild_v1",
    )
...
def main() -> None:
    ...
    summary = build_wave52_incremental_audit(**vars(args))
