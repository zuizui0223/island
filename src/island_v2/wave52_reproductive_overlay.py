"""Build the lossless Wave52 support-two reproductive overlay."""
...
from island_v2.wave52_reproductive_checkpoint import (
    BASELINE_WAVE51_RUN_ID,
    FORMAL_WAVE33_RUN_ID,
)


def build_wave52_overlay(
    *,
    baseline_csv: Path,
    ...
    with tempfile.TemporaryDirectory(prefix="wave52-overlay-") as temporary:
        ...
        mapping = {
            "wave45_species_axis_coverage.csv.gz": "wave52_species_axis_coverage.csv.gz",
            "wave45_resolved_direct_species_trait.csv.gz": "wave52_resolved_direct_species_trait.csv.gz",
            "wave45_new_validated_low_species_trait.csv.gz": "wave52_new_validated_low_species_trait.csv.gz",
            "wave45_new_trait_specific_genus_rule_audit.csv.gz": "wave52_new_trait_specific_genus_rule_audit.csv.gz",
            "wave45_change_audit.csv.gz": "wave52_change_audit.csv.gz",
            "wave45_external_congener_resolved_species_trait.csv.gz": "wave52_external_congener_resolved_species_trait.csv.gz",
            "wave45_external_congener_source_conflicts.csv.gz": "wave52_external_congener_source_conflicts.csv.gz",
        }
    ...
    delta = summary["delta"]
    delta["resolved_wave52_direct_species_trait"] = delta.pop(
        "resolved_wave45_direct_species_trait"
    )
    delta["resolved_wave52_direct_species_axis"] = delta.pop(
        "resolved_wave45_direct_species_axis"
    )
    summary.update(
        {
            "contract": "wave52_support_two_reproductive_lossless_overlay_v1",
            "baseline_formal_run_id": BASELINE_WAVE51_RUN_ID,
            "formal_wave33_run_id": FORMAL_WAVE33_RUN_ID,
            "source_lineages": [
                "huntington-isi:2005-04",
                "url:https://fr.wikipedia.org/wiki/Helichrysum_italicum",
                "sanbi-plantzafrica:triumfetta-annua:2015-11",
            ],
    ...
    summary["checks"].update(
        {
            ...
            "formal_wave51_immediate_baseline_pinned": True,
            ...
            "completed_axis_enrichment_not_double_counted": True,
            "reproductive_traits_not_interchanged": True,
        }
    )
    summary["artifact_sha256"] = {
        path.name: _sha256(path) for path in sorted(output_dir.glob("wave52_*.csv.gz"))
    }
    summary_path = output_dir / "wave52_coverage_summary.json"
...
def main() -> None:
    ...
    summary = build_wave52_overlay(**vars(args))
