...
from island_v2.global_trait_campaign import load_config, load_species_master
...
CONTRACT_VERSION = "public_web_9col_shards_v3"
CHECKPOINT_COLUMNS = [
    "species",
    "genus",
    "family",
...
def load_campaign_master(master_csv: Path, config_path: Path) -> pd.DataFrame:
    """Use the existing global campaign scope so the denominator stays 105,612."""
    return load_species_master(master_csv, load_config(config_path))
...
def run_shard(
    *,
    ...
    master = load_campaign_master(master_csv, config_path)
...
def plan_command(
    ...
    expected_species: int = typer.Option(105612, min=0),
) -> None:
