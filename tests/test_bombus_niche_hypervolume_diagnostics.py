...
def test_hypervolume_boundary_is_calibrated_to_half_membership() -> None:
    probe = pd.DataFrame(
        [{"island_id": "probe", "bombus_species": "Bombus alpha", "bio1": 11.0, "bio12": 880.0}]
    )
    scored = score_niche_hypervolumes(
        _training(), probe, environment_columns=["bio1", "bio12"]
    ).iloc[0]
...
def test_univariate_extrapolation_is_explicit() -> None:
    targets = pd.DataFrame(
        [
            {"island_id": "inside_axes", "bombus_species": "Bombus alpha", "bio1": 11.0, "bio12": 880.0},
            {"island_id": "outside_axes", "bombus_species": "Bombus alpha", "bio1": 30.0, "bio12": 2500.0},
        ]
    )
    result = score_niche_hypervolumes(
        _training(), targets, environment_columns=["bio1", "bio12"]
    ).set_index("island_id")
...
def test_unresolved_rows_keep_diagnostic_schema() -> None:
    target = pd.DataFrame(
        [{"island_id": "unresolved", "bombus_species": "Bombus alpha", "bio1": 10.0, "bio12": 800.0}]
    )
    result = score_niche_hypervolumes(
        _training().head(4), target, environment_columns=["bio1", "bio12"], min_occurrences=10
    ).iloc[0]
