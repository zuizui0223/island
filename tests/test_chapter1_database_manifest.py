...
def _mock_manifest(species_axis: Path, axes: list[str], registry: Path | None = None) -> Chapter1DatabaseManifest:
    db: dict[str, object] = {
        "database_id": "test",
        "version": "2.0.0",
...
def test_species_axis_validation_is_versioned_and_hash_locked(tmp_path: Path) -> None:
    ...
    manifest = _mock_manifest(path, axes)
...
def test_database2_rights_registry_covers_all_resolved_lineages(tmp_path: Path) -> None:
    ...
    ).to_csv(registry, index=False, compression="gzip")
