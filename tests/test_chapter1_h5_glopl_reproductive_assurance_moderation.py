...
def test_build_trait_assignments_excludes_ambiguous_states() -> None:
    ledger = pd.DataFrame(
        {
            "accepted_species": ["A a", "B b", "C c", "D d", "E e", "F f"],
            "axis": ["reproductive_assurance"] * 6,
            "trait_name": [
    ...
    assert got == {
        ("a a", "self_compatibility", 1),
        ("b b", "self_compatibility", 0),
        ("d d", "selfing_mating_system", 1),
        ("f f", "autonomous_selfing", 0),
    }


def test_summarize_trait_support_requires_both_classes_and_offshore_support() -> None:
    rows = pd.DataFrame(
        {
            "trait": ["self_compatibility"] * 8,
    ...
    trait = summary["self_compatibility"]
...
def test_classification_requires_negative_interaction_and_positive_unprotected_slope() -> None:
    primary = {
        "evaluable": True,
        "unprotected_distance_slope": 0.30,
        "distance_by_trait_interaction": -0.20,
        "interaction_one_sided_negative_p": 0.02,
    }
    sensitivities = {
        "supplemental_only": {"distance_by_trait_interaction": -0.10},
        "no_zero_constant": {"distance_by_trait_interaction": -0.05},
    }
    assert classify_trait_result(primary, sensitivities)["supported"] is True
    primary["unprotected_distance_slope"] = -0.01
