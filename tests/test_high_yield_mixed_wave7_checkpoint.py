...
from island_v2.high_yield_mixed_wave7_checkpoint import (
    SOURCE_GROUP,
    SOURCES,
    reviewed_rows,
)
...
def test_rows_are_exact_trait_specific_direct_evidence() -> None:
    ...
    assert set(evidence["language"]) == {"en", "es", "fr", "pt"}
...
def test_queue_potential_is_diagnostic_only() -> None:
    assert sum(int(source["potential"]) for source in SOURCES) == 232
    evidence = pd.DataFrame(reviewed_rows()).fillna("")
    gastrodia = evidence.loc[
        evidence["accepted_species"].eq("Gastrodia kuroshimensis")
    ].iloc[0]
    assert gastrodia["trait_name"] == "autonomous_selfing_capacity"
    assert "drop directly onto the surface of the stigma" in gastrodia["source_excerpt"]
    assert not evidence["trait_name"].eq("cleistogamy").any()


def test_explicit_synonym_is_not_fuzzy_matched() -> None:
    evidence = pd.DataFrame(reviewed_rows()).fillna("")
    ceodes = evidence.loc[evidence["accepted_species"].eq("Ceodes grandis")].iloc[0]
    assert ceodes["evidence_scope"] == "synonym_direct"
    assert ceodes["matched_page_name"] == "Pisonia grandis"
    assert ceodes["name_match_method"] == "exact_synonym"
    assert "Synonyme : Ceodes grandis" in ceodes["source_excerpt"]
...
def test_committed_combined_checkpoint_passes_individual_review_gate() -> None:
    ...
    assert len(evidence) == len(audit) == len(accepted) == 1_936
