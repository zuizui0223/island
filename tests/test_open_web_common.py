...
def test_reviewed_exclusion_table_blocks_known_species_context_and_ontology_errors() -> None:
    ...
    assert (
        ...
        "self_incompatibility",
        "SI",
        "url:https://europepmc.org/article/MED/12582529",
    ) in keys
    assert (
        "*",
        "autonomous_selfing_capacity",
...
def test_review_promotion_can_run_on_pr_with_exact_pinned_artifacts() -> None:
    ...
    assert (
        "targeted_support2_wave19_checkpoint_20260820/"
        "combined_curated_evidence_20260820.csv" in workflow
    )
    ...
    assert (
        "targeted_support2_wave19_checkpoint_20260820/"
        "combined_curated_manual_audit_20260820.csv" in workflow
    )
